...
### Added
- Hero App iFrame Padded Card Border and Phone Ratio (`inc/hero.php`, `assets/css/sections/hero-overlap.css`, `assets/js/customizer-preview-header-footer.js`): Enhanced hero iframe mockup presentation with a dark padded card frame border matching the showcase card style. Removed raw browser iframe borders, enforced 9:19.5 mobile phone aspect ratio with 560px min-height, added flex centering in split and centered hero layouts, excluded phone mockup elements from in-canvas contenteditable mode, and added live `wp.customize` postMessage synchronization for `mh_hero_iframe_url`.
- Customizer Footer Navigation Menu Selector (`inc/customizer/sections/section-footer.php`, `inc/header-footer.php`, `assets/css/header-footer.css`, `assets/js/customizer-preview-header-footer.js`): Added `mh_footer_menu` setting and select control in Customizer Footer Settings with selective refresh. Users can choose any user-created navigation menu or default to theme location fallbacks (Primary, Footer 1, Footer 2, Footer 3, Footer 4). Implemented `mh_render_footer_nav_menu()` helper to dynamically render WordPress menus across all 20 footer archetypes, replacing hardcoded static links in terminal, status, and colophon layouts while preserving horizontal flex alignment and list resets.
...
### Fixed
- CLI Terminal Command Copy to Clipboard (`assets/js/header-footer.js`, `assets/css/header-footer.css`): Fixed clipboard copy failure where the button failed to write terminal text to the clipboard in restricted contexts and subframes. Updated handler to dynamically read active terminal text from `.mh-cli-text`, implemented multi-tiered clipboard writing (native `navigator.clipboard`, parent window clipboard delegation for Customizer subframes, and visible in-viewport `execCommand` fallback), added green `.copied` visual state feedback, and prevented timeout race conditions.
- Developer Terminal CLI Copy Button and Footer Hierarchy (`inc/header-footer.php`, `assets/js/header-footer.js`, `assets/js/customizer-preview-header-footer.js`): Fixed an issue where the developer terminal copy button used an inline `onclick` handler containing an arrow function (`=>`). WordPress `wptexturize` treated the `>` character as the end of the button HTML tag, texturizing subsequent code and quotes into entities (`&#8217;`, `&#8221;`) and leaving the `onclick` attribute unclosed in the browser. This caused the closing tags of the terminal card to be swallowed and appended the entire footer navigation row (brand logo, links, and copyright text) inside the copy button within the mock terminal box. Extracted clipboard copy logic into an unobtrusive event listener in `header-footer.js` with fallback support, converted the copy button to use `data-copy-text` with an explicit `aria-label`, and synchronized Customizer live preview bindings.
