...
### Fixed
- CLI Terminal Command Copy to Clipboard (`assets/js/header-footer.js`, `assets/js/customizer-preview-header-footer.js`, `inc/header-footer.php`, `functions.php`): Fixed clipboard copy failure where the button failed to write terminal text to the clipboard. Replaced asynchronous gesture-delayed fallback with synchronous in-gesture `execCommand` execution, direct `window.mhCopyCliText` button binding, parent window delegation, dynamic extraction of text from `.mh-cli-text` / `.mh-terminal-code`, and `filemtime` cache busting on script enqueues.
